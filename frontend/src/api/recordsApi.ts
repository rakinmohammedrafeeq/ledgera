...
export const recordsApi = {
  async list(params: RecordsQueryParams): Promise<Page<FinancialRecordResponse>> {
    const response = await apiClient.get<Page<FinancialRecordResponse>>('/api/records', { params })
    return response.data
  },

  async getById(id: number): Promise<FinancialRecordResponse> {
    const response = await apiClient.get<FinancialRecordResponse>(`/api/records/${id}`)
    return response.data
  },

  async create(body: FinancialRecordRequest): Promise<FinancialRecordResponse> {
    const response = await apiClient.post<FinancialRecordResponse>('/api/records', body)
    return response.data
  },

  async update(id: number, body: FinancialRecordRequest): Promise<FinancialRecordResponse> {
    const response = await apiClient.put<FinancialRecordResponse>(`/api/records/${id}`, body)
    return response.data
  },
